...
export default function BorrowingForm({ onSubmit, equipmentId }) {
  ...
  return (
    <form onSubmit={(e) => { e.preventDefault(); if (count<=0 || durationHours<=0) return; onSubmit?.({ equipmentId, durationHours, count }); }} style={{ display: "flex", gap: 8, alignItems: "center", flexWrap:'wrap' }}>
      ...
      <input className="input" type="number" min={1} placeholder="Count" value={count} onChange={(e)=> setCount(Math.max(1, Number(e.target.value)))} />
